...
if __name__ == '__main__':
    attn = Attention(lib.ENCODER_DIM,lib.DECODER_DIM,attention_method="concat",attention_dim=lib.ATTENTION_DIM,attention_type="local")
    cnn = MyEncoderCNN()
    for idx,(image,target) in enumerate(get_data_loader()):
        fake_hidden_state = torch.tensor(np.arange(2560).reshape(5,512),dtype=torch.float32)
        print(attn(fake_hidden_state,cnn(image)))
        break
