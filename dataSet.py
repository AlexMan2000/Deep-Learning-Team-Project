...
from word_dictionary import MyVocab,Glove,Word2Vec



#Doing Data Processing
class Flickr8K(Dataset):
    def __init__(self,root_path,captions_path,transform=None,train = True,embed_vocab="randomize"):
        """
        ...
        assert embed_vocab in ["randomize","Glove","Word2Vec"]
...
embedding_matrix = dataset.word_dictionary
...
def show_image(img_pixel,label = None,img_name=None):
    """
    ...
    #Un-normalize
    img_pixel[0] = img_pixel[0] * 0.229
    img_pixel[1] = img_pixel[1] * 0.224
    img_pixel[2] = img_pixel[2] * 0.225
    img_pixel[0] += 0.485
    img_pixel[1] += 0.456
    img_pixel[2] += 0.406
    ...
    # if label:
    #     plt.title(label)
    plt.savefig("./static/{}{}".format("~original_image~", img_name))
    plt.pause(0.001)
