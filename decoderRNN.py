...
class MyDecoderRNN(nn.Module):
    ...
    def generate_caption(self,image_features,max_len=20,vocabulary=dataset.word_dictionary):
        ...
        if self.attention_type == "none":
            ...
            for i in range(max_len):
                ...
                predicted_word_index = output.argmax(dim=1)
                ...
                inputs = self.embedding_layer(predicted_word_index.unsqueeze(0))
...
if __name__ == '__main__':
    for idx,(image,target) in enumerate(get_data_loader()):
        cnn = MyEncoderCNN()
        rnn = MyDecoderRNN(VOCAB_DIM,lib.EMBED_DIM,lib.ENCODER_DIM,lib.DECODER_DIM,GRU=True)
